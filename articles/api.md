...
> production data. See [Sandbox Mode](#sandbox-mode) below for how it
> works and how it is turned off for deployment.
...
# One real lookup while sandbox is otherwise on
orgs <- fetch_org_options(sandbox = FALSE)
```
...
`csiapps` includes a local sandbox that emulates the warehouse endpoints
above, so ingestion pipelines can be developed and tested with no
network access, no authentication, and no risk of writing test data to
the production warehouse. Sandbox requests are routed to an in-memory
warehouse that validates records against a registered JSON schema (using
the `jsonvalidate` package) and stores accepted payloads locally.

**Sandbox mode is enabled by default.** Every one of the
[`make_request()`](https://csiontario.github.io/csiapps/reference/make_request.md)
calls shown earlier in this vignette runs against the local sandbox
until sandbox mode is explicitly turned off. Every sandboxed request
also prints a `csiapps sandbox:` message, so it is always visible that
no real API call was made.

To turn sandbox mode off — for example, when deploying a finished app to
production — set the R option to `FALSE`, or the `CSIAPPS_ENV`
...
The `sandbox` argument of
[`make_request()`](https://csiontario.github.io/csiapps/reference/make_request.md)
can also override the global setting for an individual call,
e.g. `make_request(..., sandbox = FALSE)`.

#### Best Practices

Follow these two practices so that apps transition cleanly from sandbox
to production:
...
1.  **Do not set the `csiapps.sandbox` option (or the `sandbox`
    ...
    development without any configuration. Hard-coding
    `options(csiapps.sandbox = TRUE)` in committed code would override
    the deployer’s attempt to turn sandbox off, causing the deployed app
    to silently serve sandbox data.

2.  **Turn sandbox off only at deployment, in a location the deployer
    controls.** Sandbox mode should be disabled when the finished app is
    deployed — typically by setting `options(csiapps.sandbox = FALSE)`
    in the app’s `global.R` (or an `Rprofile.site`) at deployment time,
    or by setting `CSIAPPS_ENV=production` in the production
    environment. Keeping this out of your app code means the same code
    runs in the sandbox during development and against production once
...
From here, the workflow is *identical* to the production examples above
— the same
[`make_request()`](https://csiontario.github.io/csiapps/reference/make_request.md)
calls, with the dummy uuid in place of `Sys.getenv("SOURCE_UUID")`:
...
describing exactly which records failed and why. When code is ready for
production, disable sandbox mode and swap in the real source uuid — no
other changes are needed.

#### Inspecting and Resetting the Sandbox

Accepted payloads are written as pretty-printed JSON files, one folder
per data source, so the exact JSON that would have been sent to the API
can be inspected:
