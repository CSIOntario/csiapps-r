...
test_that("create_sport_org generates an unused 3-digit id when none is given", {
  local_clean_sandbox()
  org <- suppressMessages(create_sport_org())
  expect_true(org$id >= 100 && org$id <= 999)
  expect_true(nzchar(org$name))
})

test_that("create_sport_org honours a supplied id and rejects collisions", {
  local_clean_sandbox()
  suppressMessages(create_sport_org(id = 321))
  expect_error(suppressMessages(create_sport_org(id = 321)), "already exists")
})

test_that("create_profile requires an existing sport org", {
  local_clean_sandbox()
  expect_error(suppressMessages(create_profile(2, sport_org_id = 999)), "does not exist")
})

test_that("fetch_org_options returns seeded orgs as label/value pairs", {
  local_clean_sandbox()
  suppressMessages(create_sport_org(id = 100))
  opts <- suppressMessages(fetch_org_options(sandbox = TRUE))
  expect_length(opts, 1)
  expect_identical(opts[[1]]$value, 100L)
  expect_true(nzchar(opts[[1]]$label))
})

test_that("fetch_profiles returns prod-shaped athletes filtered by sport_org_id", {
  local_clean_sandbox()
  suppressMessages(create_sport_org(id = 100))
  suppressMessages(create_sport_org(id = 200))
  suppressMessages(create_profile(3, sport_org_id = 100))
  suppressMessages(create_profile(2, sport_org_id = 200))
...
test_that("ingested records link to a registered athlete via subject_field", {
  local_clean_sandbox()
  suppressMessages(create_sport_org(id = 100))
  suppressMessages(create_profile(1, sport_org_id = 100))  # athlete id 1
...
test_that("an athlete's sport is coherent with its sport org", {
  local_clean_sandbox()
  org <- suppressMessages(create_sport_org(id = 100))
  suppressMessages(create_profile(1, sport_org_id = 100))
  p <- suppressMessages(fetch_profile(profile_id = 1, sandbox = TRUE))
  expect_true(nzchar(org$sport))
  expect_identical(p$sport$name, org$sport)
})
...
test_that("record subjects resolve at read time so late profiles backfill", {
  ...
  suppressMessages(create_profile(1, sport_org_id = 100))  # athlete id 2
...
test_that("create_* warn when not in sandbox mode", {
  ...
  expect_warning(suppressMessages(create_sport_org(id = 100)), "sandbox")
  expect_warning(suppressMessages(create_profile(1, sport_org_id = 100)), "sandbox")
})

test_that("create_sport_org rejects invalid supplied ids", {
  local_clean_sandbox()
  expect_error(suppressMessages(create_sport_org(id = 0)))
  expect_error(suppressMessages(create_sport_org(id = -5)))
  expect_error(suppressMessages(create_sport_org(id = 1.5)))
})
