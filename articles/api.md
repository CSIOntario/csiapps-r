...
Enable sandbox mode globally with an R option, or per session with the
`CSIAPPS_ENV=sandbox` environment variable:
...
Every sandboxed request prints a `csiapps sandbox:` message so it is
always visible that no real API call was made. Individual calls can also
override the global setting with the `sandbox` argument,
